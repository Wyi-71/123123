...
OUTPUT_DIR = os.path.join(DATA_ROOT, 'results_rigorous_avg')
...
def main():
    ...
    for name, model_fn in models_config.items():
        ...
        model, avg_acc = train_model(model, s_train_loader, s_test_loader, EPOCHS_SOURCE)
        expert_weights[name] = model.state_dict()
        print(f"✅ {name} 在严格防泄露的 0HP 上训练完成! 平均真实泛化准确率: {avg_acc:.2f}%")
        results.append({'Model': name, 'Load': '0HP', 'Accuracy': avg_acc})
    ...
    for load in ['1HP', '2HP', '3HP']:
        ...
        for name in models_config.keys():
            ...
            # 直接在目标负载上测试 (不进行任何学习)
            # 因为零样本测试只测一次，准确率就这一个值，所以在这个层面平均准确率和单次准确率是等价的。
            # 主要的区别在于 0HP 上的基准是用平均。
    ...
    df.to_csv(os.path.join(OUTPUT_DIR, 'rigorous_transfer_results_avg.csv'), index=False)
    print(f"\n📊 严谨实验结果已保存至 {OUTPUT_DIR}\\rigorous_transfer_results_avg.csv")
...
def plot_line_comparison(df):
    ...
    plt.title('Rigorous Zero-Shot Generalization (Base: 0HP Train, Avg Acc)', fontsize=14, fontweight='bold', pad=15)
    plt.xlabel('Load Condition', fontsize=12)
    plt.ylabel('Average Valid/Test Accuracy (%)', fontsize=12)
    ...
    save_path = os.path.join(OUTPUT_DIR, 'rigorous_comparison_line_avg.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    print(f"📈 严谨版平均准确率最终对比折线图已生成: {save_path}")
