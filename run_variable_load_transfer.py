...
def main():
    ...
    df.to_csv(os.path.join(DATA_ROOT, 'variable_load_zero_shot_results.csv'), index=False)
    print("\n📊 结果已保存至 variable_load_zero_shot_results.csv")
...
def plot_line_comparison(df):
    ...
    plt.title('Zero-Shot Generalization Performance Contrast (Base: 0HP Train, Best Acc)', fontsize=14, fontweight='bold', pad=15)
    ...
    save_path = os.path.join(DATA_ROOT, 'variable_load_comparison_line.png')
