...
THEME = {
    ...
    "nav_active_bg": "#2563eb",    # 导航按钮激活态
    "nav_active_fg": "#ffffff",
    "nav_inactive_bg": "#e2e8f0",
    "nav_inactive_fg": "#334155",
}
...
class CWRUDiagnosisApp:
    def __init__(self, root):
        ...
        # 缓存 STFT 绘图数据
        self._stft_data = None   # (f, t, Zxx)
        self._probs_data = None  # probs array
        # 日志缓存
        self._log_lines = []
    ...
    def build_architecture(self):
        ...
        # 核心模块 1：系统总控侧边栏
        # ====================
        sidebar = tk.Frame(self.root, bg=THEME["sidebar_bg"], width=380)
        sidebar.pack(side=tk.LEFT, fill=tk.Y)
        sidebar.pack_propagate(False)

        # 1.1 品牌/标题区
        title_frame = tk.Frame(sidebar, bg=THEME["sidebar_bg"], pady=30, padx=25)
        title_frame.pack(fill=tk.X)
        tk.Label(title_frame, text="CWRU",
                 font=("Consolas", 38, "bold"),
                 bg=THEME["sidebar_bg"], fg=THEME["primary"]).pack(anchor=tk.W)
        ...
        tk.Label(control_frame, text="1. 算法计算节点引擎",
                 font=("Microsoft YaHei UI", 13, "bold"),
                 bg=THEME["sidebar_bg"], fg=THEME["sidebar_fg"]).pack(anchor=tk.W, pady=(0, 10))

        self.model_var = tk.StringVar(value="SimpleCNN (CNN)")
        self.model_combo = ttk.Combobox(control_frame, textvariable=self.model_var,
                                        values=["SimpleCNN (CNN)", "ResNet18",
                                                "CBAM-ResNet18 (时空注意力)",
                                                "CNN-LSTM (时频特征序列)"],
                                        state="readonly",
                                        font=("Microsoft YaHei UI", 12))
        self.model_combo.pack(fill=tk.X, pady=(0, 15), ipady=5)
        self.model_combo.bind("<<ComboboxSelected>>", lambda e: self.load_model())
        ttk.Button(control_frame, text="挂载引擎",
                   style="Outline.TButton", command=self.load_model).pack(fill=tk.X, ipady=4)
        ...
        tk.Label(col_frame, text="2. 数据矩阵通道切换",
                 font=("Microsoft YaHei UI", 13, "bold"),
                 bg=THEME["sidebar_bg"], fg=THEME["sidebar_fg"]).pack(anchor=tk.W, pady=(0, 10))
        ...
        self.column_spinbox = ttk.Spinbox(c_inner, from_=0, to=0,
                                          textvariable=self.column_var,
                                          width=10, state="disabled",
                                          font=("Consolas", 14))
        self.column_spinbox.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10), ipady=5)
        self.btn_change_col = ttk.Button(c_inner, text="切换",
                                         style="Outline.TButton",
                                         command=self.change_column,
                                         state=tk.DISABLED)
        self.btn_change_col.pack(side=tk.RIGHT, ipady=4)

        tk.Frame(sidebar, bg=THEME["sidebar_accent"], height=1).pack(fill=tk.X, padx=25)

        # 1.4 快捷功能按钮区 —— STFT 图像 & 日志 弹窗跳转
        action_frame = tk.Frame(sidebar, bg=THEME["sidebar_bg"], padx=25, pady=25)
        action_frame.pack(fill=tk.X)
        tk.Label(action_frame, text="3. 详细数据查看",
                 font=("Microsoft YaHei UI", 13, "bold"),
                 bg=THEME["sidebar_bg"], fg=THEME["sidebar_fg"]).pack(anchor=tk.W, pady=(0, 12))

        self.btn_stft = tk.Button(action_frame,
                                  text="查看 STFT 时频图",
                                  font=("Microsoft YaHei UI", 13, "bold"),
                                  bg=THEME["primary"], fg="#ffffff",
                                  activebackground="#1d4ed8", activeforeground="#ffffff",
                                  relief=tk.FLAT, cursor="hand2",
                                  command=self.show_stft_window)
        self.btn_stft.pack(fill=tk.X, ipady=8, pady=(0, 10))

        self.btn_log = tk.Button(action_frame,
                                 text="查看系统日志",
                                 font=("Microsoft YaHei UI", 13, "bold"),
                                 bg="#475569", fg="#ffffff",
        ...
        self.btn_log.pack(fill=tk.X, ipady=8)

        # 底部版权
        tk.Label(sidebar, text="v2.0  Optimized UI",
                 font=("Consolas", 11),
                 bg=THEME["sidebar_bg"], fg=THEME["sidebar_accent"]).pack(side=tk.BOTTOM, pady=15)
        ...
        # 2.1 顶级控制工具栏
        header_frame = tk.Frame(main_content, bg=THEME["card_bg"], height=100)
        header_frame.pack(fill=tk.X, padx=25, pady=25)
        header_frame.pack_propagate(False)

        ttk.Button(header_frame, text="打开数据集文件 (.mat)",
                   style="Primary.TButton",
                   command=self.open_file).pack(side=tk.LEFT, padx=25, pady=25, ipady=8, ipadx=18)
        ...
        tk.Label(file_info_frame, text="当前加载轨道:",
                 font=("Microsoft YaHei UI", 11),
                 bg=THEME["card_bg"], fg=THEME["text_sub"]).pack(anchor=tk.W)
        self.file_label = tk.Label(file_info_frame,
                                   text="< 尚未导入任何数据文件 >",
                                   font=("Consolas", 15, "bold"),
                                   bg=THEME["card_bg"], fg=THEME["text_main"])
        self.file_label.pack(anchor=tk.W, pady=(2, 0))

        # 2.1.1 右侧模块状态信号灯
        self.status_var = tk.StringVar(value="SYSTEM READY")
        self.status_badge = tk.Label(header_frame, textvariable=self.status_var,
                                     font=("Consolas", 14, "bold"),
                                     bg="#e2e8f0", fg=THEME["text_main"],
                                     padx=22, pady=10)
        ...
        # 数据包参数板
        c1 = tk.Frame(cards_frame, bg=THEME["card_bg"], padx=25, pady=22)
        c1.grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        tk.Label(c1, text="分析目标源属性",
                 font=("Microsoft YaHei UI", 12),
                 bg=THEME["card_bg"], fg=THEME["text_sub"]).pack(anchor=tk.W)
        self.card1_val = tk.Label(c1, text="CWRU Base",
                                  font=("Consolas", 22, "bold"),
                                  bg=THEME["card_bg"], fg=THEME["text_main"])
        self.card1_val.pack(anchor=tk.W, pady=(6, 0))
        self.card1_sub = tk.Label(c1, text="Fs Sample Rate: -- kHz",
                                  font=("Consolas", 13),
                                  bg=THEME["card_bg"], fg=THEME["text_sub"])
        self.card1_sub.pack(anchor=tk.W)

        # 模型推演核心结果
        c2 = tk.Frame(cards_frame, bg=THEME["card_bg"], padx=25, pady=22)
        c2.grid(row=0, column=1, sticky="nsew", padx=12)
        tk.Label(c2, text="神经网络推演预测结论",
                 font=("Microsoft YaHei UI", 12),
                 bg=THEME["card_bg"], fg=THEME["text_sub"]).pack(anchor=tk.W)
        self.card2_val = tk.Label(c2, text="---",
                                  font=("Microsoft YaHei UI", 22, "bold"),
                                  bg=THEME["card_bg"], fg=THEME["text_main"])
        self.card2_val.pack(anchor=tk.W, pady=(4, 0))
        self.card2_sub = tk.Label(c2, text="等待执行运算 ...",
                                  font=("Microsoft YaHei UI", 13),
                                  bg=THEME["card_bg"], fg=THEME["text_sub"])
        self.card2_sub.pack(anchor=tk.W)

        # 预测置信指标
        c3 = tk.Frame(cards_frame, bg=THEME["card_bg"], padx=25, pady=22)
        c3.grid(row=0, column=2, sticky="nsew", padx=(12, 0))
        tk.Label(c3, text="模型输出置信度",
                 font=("Microsoft YaHei UI", 12),
                 bg=THEME["card_bg"], fg=THEME["text_sub"]).pack(anchor=tk.W)
        self.card3_val = tk.Label(c3, text="0.00 %",
                                  font=("Consolas", 34, "bold"),
                                  bg=THEME["card_bg"], fg=THEME["primary"])
        self.card3_val.pack(anchor=tk.W, pady=(0, 0))

        # 2.3 概率分布柱状图容器 (主面板只保留概率柱状图)
        ...
        tk.Label(header_plot, text="模型输出概率空间分布",
                 font=("Microsoft YaHei UI", 14, "bold"),
                 bg=THEME["card_bg"], fg=THEME["text_main"]).pack(side=tk.LEFT)
    ...
    def log(self, message):
        ...
        # 如果日志弹窗已打开，实时更新
        if hasattr(self, '_log_text_widget') and self._log_text_widget is not None:
            try:
                self._log_text_widget.insert(tk.END, message + "\n")
    ...
    # 弹窗：STFT 时频图
    # =================
    def show_stft_window(self):
        win = tk.Toplevel(self.root)
        win.title("STFT 时频特征解析图")
        ...
        # 标题栏
        hdr = tk.Frame(win, bg=THEME["sidebar_bg"], height=60)
        hdr.pack(fill=tk.X)
        hdr.pack_propagate(False)
        tk.Label(hdr, text="STFT Time-Frequency Envelope",
                 font=("Microsoft YaHei UI", 16, "bold"),
                 bg=THEME["sidebar_bg"], fg="#ffffff").pack(side=tk.LEFT, padx=25, pady=15)
        tk.Button(hdr, text="关闭", font=("Microsoft YaHei UI", 12, "bold"),
                  bg=THEME["danger"], fg="#ffffff", relief=tk.FLAT,
                  command=win.destroy, cursor="hand2").pack(side=tk.RIGHT, padx=25, pady=12, ipadx=15)

        frame = tk.Frame(win, bg=THEME["card_bg"])
        frame.pack(fill=tk.BOTH, expand=True, padx=15, pady=15)

        if self._stft_data is not None:
            f, t, Zxx = self._stft_data
            plt.style.use('default')
            plt.rcParams['font.family'] = 'serif'
            plt.rcParams['font.serif'] = ['Times New Roman', 'DejaVu Serif', 'SimSun']
            fig, ax = plt.subplots(1, 1, figsize=(9, 5.5), dpi=100, facecolor=THEME["card_bg"])
            fig.patch.set_facecolor(THEME["card_bg"])
            ax.set_facecolor("#f8fafc")
            c = ax.pcolormesh(t, f, np.abs(Zxx), shading='gouraud', cmap='jet')
            ax.set_title('STFT Extracted Feature Responses',
                         color=THEME["text_main"], fontweight='bold', fontsize=14)
            ax.set_ylabel('Frequency Space [Hz]', color=THEME["text_main"],
                          fontweight='heavy', fontsize=12)
            ax.set_xlabel('Time Sequence [sec]', color=THEME["text_main"],
                          fontweight='heavy', fontsize=12)
            ...
            cb.ax.tick_params(labelsize=11, colors=THEME["text_main"])
            ...
            tk.Label(frame, text="暂无 STFT 数据\n请先加载数据文件并完成诊断",
                     font=("Microsoft YaHei UI", 16),
                     bg=THEME["card_bg"], fg=THEME["text_sub"]).pack(expand=True)
    ...
    def show_log_window(self):
        ...
        win.configure(bg=THEME["term_bg"])
        win.transient(self.root)
        win.grab_set()
        ...
        hdr.pack(fill=tk.X)
        hdr.pack_propagate(False)
        tk.Label(hdr, text=">_ 后台系统终端输出",
                 font=("Microsoft YaHei UI", 16, "bold"),
                 bg=THEME["sidebar_bg"], fg="#ffffff").pack(side=tk.LEFT, padx=25, pady=15)

        btn_frame = tk.Frame(hdr, bg=THEME["sidebar_bg"])
        btn_frame.pack(side=tk.RIGHT, padx=25, pady=12)
        tk.Button(btn_frame, text="清空日志", font=("Microsoft YaHei UI", 11, "bold"),
                  bg=THEME["warning"], fg="#ffffff", relief=tk.FLAT,
                  command=lambda: self._clear_log(log_text), cursor="hand2").pack(side=tk.LEFT, padx=(0, 10), ipadx=10)
        tk.Button(btn_frame, text="关闭", font=("Microsoft YaHei UI", 11, "bold"),
                  bg=THEME["danger"], fg="#ffffff", relief=tk.FLAT,
                  command=lambda: self._close_log_window(win), cursor="hand2").pack(side=tk.LEFT, ipadx=15)

        log_text = tk.Text(win, bg=THEME["term_bg"], fg=THEME["term_fg"],
                           font=("Consolas", 12), borderwidth=0, padx=18, pady=18,
        ...
        # 保存引用以便实时更新
        self._log_text_widget = log_text

    def _clear_log(self, text_widget):
        self._log_lines.clear()
        text_widget.delete("1.0", tk.END)

    def _close_log_window(self, win):
        self._log_text_widget = None
        win.destroy()

    # =================
    # 后台数学绘图实现 (主面板仅显示概率柱状图)
    # =================
    def _init_empty_plot(self):
        ...
        fig, ax = plt.subplots(1, 1, figsize=(10, 5), dpi=100, facecolor=THEME["card_bg"])
        fig.patch.set_facecolor(THEME["card_bg"])

        ax.set_facecolor("#f8fafc")
        ax.tick_params(colors='#1e293b', labelsize=12)
        for spine in ax.spines.values():
            spine.set_color('#cbd5e1')
            spine.set_linewidth(1.5)

        ax.set_title("Model Output Probability Space",
                     color=THEME["text_main"], fontweight='bold', fontsize=14)
        ax.set_xlim(-0.5, 3.5)
        ax.set_ylim(0, 100)
        ax.set_xticks(range(4))
        ax.set_xticklabels(['Normal', 'Inner', 'Outer', 'Ball'],
                           rotation=0, fontsize=13)
        ax.text(0.5, 0.5, "[ AWAITING SIGNAL FETCH ]",
                ha='center', va='center', color='#94a3b8',
                fontdict={'family': 'Consolas', 'size': 14},
    ...
    def _plot_results(self, f, t, Zxx, probs):
        # 缓存 STFT 数据供弹窗使用
        self._stft_data = (f, t, Zxx)
        self._probs_data = probs
        ...
        bars = ax.bar(x, probs * 100, color=colors, alpha=0.95,
                      edgecolor='#1e293b', linewidth=2.0, width=0.6)
        ax.set_title('Machine Confidence Levels (%)',
                     color=THEME["text_main"], fontweight='bold', fontsize=15)
        ax.set_xticks(x)
        ax.set_xticklabels(classes, rotation=0, color=THEME["text_main"],
                           fontweight='medium', fontsize=14)
        ax.set_ylim(0, 105)
        ax.tick_params(axis='y', labelsize=12)
        ax.grid(True, axis='y', linestyle='--', alpha=0.5, color='#94a3b8')
    ...
    def load_model(self):
        ...
        def _load():
            try:
                ...
                self.root.after(0, lambda: self.log(
                    f">> [OK] 算法挂载完毕 (Engine Online @ {self.device})"))
    ...
    def _process_file(self, file_path):
        try:
            ...
            if result[0] is None:
                ...
                self.root.after(0, lambda: self.card2_val.config(
                    text="解码被拒绝", fg=THEME["danger"]))
                self.root.after(0, lambda: self.card2_sub.config(
                    text="源文件结构不适用。"))
                return
            ...
            self.root.after(0, lambda: self._update_result_ui(
                diagnosis, confidence, probs, f, t, Zxx))

        except Exception as e:
            self.root.after(0, lambda: self.log(
                f">> [SYS ERROR] 调度栈溢出或异常抛出: {e}"))
    ...
    def preprocess_signal(self, file_path):
        try:
            ...
            if not key:
                ...
                self.root.after(0, lambda: self.log(
                    ">> [WARN] 该 Mat 数据结构非标，不包含 DE_time 轨道"))
            ...
            if len(signal) < model_length:
                self.root.after(0, lambda: self.log(
                    f">> [WARN] 时域截面不足 (len: {len(signal)} < req: {model_length})"))
                return None, None, None, None

            plot_signal = signal[:plot_length] if len(signal) > plot_length else signal
            f_plot, t_plot, Zxx_plot = scipy.signal.stft(
                plot_signal, fs=fs, nperseg=256, noverlap=128)

            model_signal = signal[:model_length]
            _, _, Zxx_model = scipy.signal.stft(
                model_signal, fs=fs, nperseg=64, noverlap=32)
            img_model = np.abs(Zxx_model)
            img_model = (img_model - img_model.min()) / \
